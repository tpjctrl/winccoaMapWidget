...
1.  [Standard custom element lifecycle](#custom-element-lifecycle)
    1.  [constructor()](#constructor)
    2.  [connectedCallback()](#connectedcallback)
    3.  [disconnectedCallback()](#disconnectedcallback)
    4.  [attributeChangedCallback()](<#attributechangedcallback()>)
    5.  [adoptedCallback()](#adoptedcallback)
2.  [Reactive update cycle](#reactive-update-cycle)
    1.  [The changedProperties map](#changed-properties)
    2.  [Triggering an update](#reactive-update-cycle-triggering)
    3.  [Performing an update](#reactive-update-cycle-performing)
    4.  [Completing an update](#reactive-update-cycle-completing)
    5.  [Handling errors in the update cycle](#errors-in-the-update-cycle)
    6.  [Implementing additional customization](#reactive-update-cycle-customizing)
3.  [External lifecycle hooks: controllers and decorators](#external-lifecycle-hooks:-controllers-and-decorators)
4.  [Server-side reactive update cycle](#server-reactive-update-cycle)
...
[Permalink to “Standard custom element lifecycle”](#custom-element-lifecycle)
...
[Permalink to “attributeChangedCallback()”](<#attributechangedcallback()>)
...
[Permalink to “adoptedCallback()”](#adoptedcallback)
...
[Permalink to “Reactive update cycle”](#reactive-update-cycle)
...
![](https://lit.dev/images/docs/components/update-1.jpg)

![](https://lit.dev/images/docs/components/update-2.jpg)

**Update**

![](https://lit.dev/images/docs/components/update-3.jpg)

**Post-Update**

![](https://lit.dev/images/docs/components/update-4.jpg)

### The changedProperties map

[Permalink to “The changedProperties map”](#changed-properties)
...
[Permalink to “TypeScript types for changedProperties”](#typescript-types-for-changedproperties)
...
[Permalink to “Changing properties during an update”](#changing-properties-during-an-update)

Changing a property _during_ the update (up to and including the `render()` method) updates the `changedProperties` map, but **doesn't** trigger a new update. Changing a property _after_ `render()` (for example, in the `updated()` method) triggers a new update cycle, and the changed property is added to a new `changedProperties` map to be used for the next cycle.

### Triggering an update

[Permalink to “Triggering an update”](#reactive-update-cycle-triggering)
...
Called when a reactive property is set. By default `hasChanged()` does a strict equality check and if it returns `true`, an update is scheduled. See [configuring `hasChanged()`](about:/docs/components/properties/#haschanged) for more information.

#### requestUpdate()

[Permalink to “requestUpdate()”](#requestUpdate)
...
Optionally, you can pass a property name and a previous value when calling `requestUpdate()`, which will be stored in the `changedProperties` map. This can be useful if you implement a custom getter and setter for a property. See [Reactive properties](https://lit.dev/docs/components/properties/) for more information about implementing custom getters and setters.
...
[Permalink to “Performing an update”](#reactive-update-cycle-performing)
...
- Updates?
  - No. Property changes inside this method do not trigger an element update.
- Call super?
  - Yes. Without a super call, the element’s attributes and template will not update.
- Called on server?
  - No.
...
The `render()` method has no arguments, but typically it references component properties. See [Rendering](https://lit.dev/docs/components/rendering/) for more information.
...
[Permalink to “Completing an update”](#reactive-update-cycle-completing)
...
[Permalink to “Handling errors in the update cycle”](#errors-in-the-update-cycle)
...
[Permalink to “Implementing additional customization”](#reactive-update-cycle-customizing)

This section covers some less-common methods for customizing the update cycle.

#### scheduleUpdate()

[Permalink to “scheduleUpdate()”](#scheduleupdate)
...
[Permalink to “getUpdateComplete()”](#getUpdateComplete)
...
[Permalink to “External lifecycle hooks: controllers and decorators”](#external-lifecycle-hooks:-controllers-and-decorators)
...
[Permalink to “Server-side reactive update cycle”](#server-reactive-update-cycle)
...
![](https://lit.dev/images/docs/components/update-server.jpg)

[Edit this page](https://github.com/lit/lit.dev/edit/main/packages/lit-dev-content/site/docs/v3/components/lifecycle.md)
